---
title: "Automated Testing with renv and GitHub Actions"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Automated Testing with renv and GitHub Actions}
...
The `resultcheck.yml` file at the root can be empty — its presence is enough
...
```r
install.packages(c("resultcheck", "testthat"))
...
* **`[ignored]` markers** — replace a volatile line in the snapshot file with
  ...
  platform.
* **`snapshot.precision`** — add a `precision` key to `resultcheck.yml` to
