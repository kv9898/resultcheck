...
For a full example workflow, see the [Get Started guide](resultcheck.html).
...
`resultcheck` identifies snapshots based on the **script name** and **snapshot name**.

This means that:

- renaming a script, or
- changing the name passed to `snapshot()`

will cause `resultcheck` to treat it as a new snapshot, rather than a modification of an existing one.

In general, once you have decided to snapshot a result, it is best to keep:
...
We recommend using Git tools such as:
...
to understand when snapshots changed and who made those changes.

This keeps snapshot files clean and focused on results, while relying on version control for provenance and collaboration.
