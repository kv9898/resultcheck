...
`resultcheck` provides lightweight helpers for checking whether
empirical results remain unchanged across code revisions, platform
differences, and package updates. Call
[`snapshot()`](https://kv9898.github.io/resultcheck/reference/snapshot.md)
on key outputs (models, tables, derived datasets) in your analysis
scripts to detect unintended result drift automatically during CI or
local testing.
...
1.  **Interactive development** — run your analysis script and call
    [`snapshot()`](https://kv9898.github.io/resultcheck/reference/snapshot.md)
    on objects you care about. On first run the snapshot is saved as a
    human-readable `.md` file. On subsequent interactive runs,
    differences are shown and you are prompted to update.

2.  **Automated testing** — wrap your script in
    ...
    [`run_in_sandbox()`](https://kv9898.github.io/resultcheck/reference/run_in_sandbox.md),
    [`snapshot()`](https://kv9898.github.io/resultcheck/reference/snapshot.md)
    switches to *testing mode*: it errors immediately if a snapshot is
    missing or has changed, making the test fail.
...
Snapshot serialization also temporarily sets `useFancyQuotes = FALSE`,
so base R quotation marks (including model-summary significance legends)
are consistent between interactive sessions and Quarto. The caller’s
setting is restored even if a snapshot method fails. Custom methods may
explicitly choose other formatting.
...
Config methods are parsed from strings in `_resultcheck.yml`, so values
like `"print + str"` or `"stats::coef"` are converted to callable
methods.
...
project root. Directories are copied recursively. Snapshot files do not
need to be listed.

### `run_in_sandbox(script_path, sandbox = NULL, ...)`

Runs an R script inside the sandbox. The working directory is set to the
sandbox, but
...
Removes the sandbox directory. Omit the argument to clean up the most
recently created sandbox.
