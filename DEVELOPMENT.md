...
generated files under `man/`. Run `Ctrl+Shift+D` afterward and review changes
to both `man/` and `NAMESPACE`.

`Ctrl+Shift+B` installs the package; it does not create the distributable
source archive. To create that archive, use:
...
GitHub state, so run it only when starting the release process.
...
already been done. Keep unreleased changes under this heading in `NEWS.md`:
...
before submission. Also wait for the GitHub Actions checks on macOS, Windows,
Ubuntu release, and R-devel.
...
`find_root()` searches upward for `_resultcheck.yml`, `resultcheck.yml`, an
...
- [RStudio package-development shortcuts](https://docs.posit.co/ide/user/ide/reference/shortcuts.html)
- [`usethis::use_release_issue()`](https://usethis.r-lib.org/reference/use_release_issue.html)
- [`usethis::use_version()` and `use_dev_version()`](https://usethis.r-lib.org/reference/use_version.html)
